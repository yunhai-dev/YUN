...
export function CommentSystem({
  ...
  // 回复列表刷新事件
  const [replyRefreshTrigger, setReplyRefreshTrigger] = useState<{commentId: number, timestamp: number} | null>(null);
          ...
          {showHealthStatus && healthStatus && (
            <div className="mt-4 p-4 bg-muted rounded-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                ...
                <div>数据库: <span className={`font-medium ${healthStatus.database ? 'text-green-600' : 'text-red-600'}`}>
                  {healthStatus.database ? '正常' : '异常'}
                </span></div>
                <div>Redis: <span className={`font-medium ${healthStatus.redis ? 'text-green-600' : 'text-red-600'}`}>
