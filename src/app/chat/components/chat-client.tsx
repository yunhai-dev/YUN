...
// 优化消息渲染组件
const ChatMessage = memo(({message, isLoading}: {message: any; isLoading: boolean}) => {
  return (
    <div>
      {message.role !== "tool" && (
...
const ChatClient = () => {
  ...
  useEffect(() => {
    ...
    const savedBaseUrl = localStorage.getItem("chat-base-url") || "https://api.suanli.cn/v1";
                ...
                >
                  <ModelSelectorTrigger asChild>
                    <PromptInputButton className="w-52 max-w-52 justify-center">
                      <span className="truncate text-center">{customModel || resolvedModel || "模型设置"}</span>
                    </PromptInputButton>
